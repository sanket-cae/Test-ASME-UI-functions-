...
# Custom CSS styling for professional engineering UI (with visible sidebar constant box)
st.markdown("""
    <style>
    .main {
...
if not st.session_state.history:
    ...
    for idx, item in enumerate(st.session_state.history):
        ...
        with st.expander(card_label, expanded=(idx == 0)):
            ...
            # Ensure eval_temps has at least initial temperature
            ...
            with sub_tab2:
                if temp_cols:
                    chart_metric = st.selectbox("Select Property to Plot:", ['Yield Strength (MPa)', 'Modulus E (GPa)', 'Thermal Cond. TC (W/m·°C)', 'Specific Heat Cp (J/kg·°C)'], key=f"chart_{idx}")
                    plot_df = full_df[['Temperature (°C)', chart_metric]].copy()
                    ...
                    if not plot_df.empty:
                        st.line_chart(plot_df.set_index('Temperature (°C)')[chart_metric], use_container_width=True)
                    else:
